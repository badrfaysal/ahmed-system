...
$direct_dues = DB::table('sy2_projects')
...
$installment_dues = DB::table('sy2_projects')
...
$supplier_debts = DB::table('sy2_supplier_debts')
    ->where('status', '!=', 'paid')
    ->selectRaw("SUM(total_amount - paid_amount) as debts")
    ->value('debts') ?? 0;

$workers_total = DB::table('sy2_band_workers')->sum('amount') ?? 0;
$workers_paid = DB::table('sy2_worker_payments')->selectRaw("SUM(amount + discount) as total_paid")->value('total_paid') ?? 0;
